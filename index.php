...
// Проверим ключ, если он есть
if (!empty($_REQUEST['auth_key'])) {

  $auth_key = md5("4888893_".intval($_REQUEST['viewer_id'])."_L2LWf3xghDMQpuNL3w8T");
  $user_key = preg_replace("/[^a-f0-9]/i", "", $_REQUEST['auth_key']);

  if ($_REQUEST['auth_key'] === "false") {

    echo json_encode(array("error" => array("message" => "Братишка, получить код можно только из приложения")));
    exit;

  } else if ($auth_key !== $user_key) {

    echo json_encode(array("error" => array("message" => "Доступ ограничен, auth_key неверный")));
    exit;
  ...
  if (isset($_POST['method']) && $_POST['method'] == "takekey") {
    ...
    if ($_POST['user_id'] > 0) {
      ...
      if ($keyChecked == true) {
        ...
        echo json_encode(array("error" => array("message" => "Код не подошел, либо пользователь не зарегистрирован")));
...
if (window.self !== window.top) {
  ...
  var home_sweet_home = /https:\/\/vk\.com\/app4888893/i.test(parentUrl);

  if (home_sweet_home == true) {
    ...
    VK.init(function() {
      ...
      document.getElementById("quick_id_placeholder").innerHTML = "id"+VK.Vars['viewer_id'];
    ...
    document.getElementsByTagName("body")[0].innerHTML = "<p style='text-align: center; padding: 10px; margin: 20px'>Wron application id</p>";

  }

} else {

  var VK = {"Vars":{"auth_key":false,"viewer_id":false}}
...
?>
    ...
    $.ajax({
      url: "index.php",
      type: "POST",
      data: {"viewer_id":VK.Vars['viewer_id'], "auth_key":VK.Vars['auth_key'], "method":"takekey"},
      dataType: "json",
    ...
    <div class="col-xs-4 col-xs-offset-4">
      <form method="post" name="id" id="quick_id_form" action="index.php" autocomplete="off">
        ...
        <div class="row" style="margin-bottom: 10px"><div class="col-xs-12" style="position: relative">
          <div style="position: absolute; top: 9px; left: 28px"><span id="quick_id_placeholder" style="color: #AAAAAA">id1234567</span></div>
